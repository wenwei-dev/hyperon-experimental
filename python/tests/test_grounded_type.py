...
class GroundedTypeTest(unittest.TestCase):
    ...
    def test_higher_func(self):
        ...
        self.assertEqual(metta.run("!((curry_num + 1) 2)"), metta.run("! 3"))

    def test_meta_types(self):
        metta = MeTTa()
        ### Basic functional types
        metta.register_atom(
            r"id_num", OperationAtom("id_num", lambda x: x, ["Number", "Number"])
        )
        metta.register_atom(
            r"as_int", OperationAtom("as_int", lambda x: x, ["Number", "Int"])
        )
        ...
        self.assertEqual(
            metta.run("!(id_num untyp)"), [metta.parse_all("(id_num untyp)")]
        )
        ...
        metta.register_atom(
            r"id_atom",
            OperationAtom(
                "id_atom", lambda x: [x], [AtomType.ATOM, AtomType.ATOM], unwrap=False
            ),
        )
        self.assertEqual(
            metta.run(
        ...
        metta.register_atom(
            r"id_poly_w",
            OperationAtom("id_poly_w", lambda x: [x], ["$t", "$t"], unwrap=False),
        )
        self.assertEqual(
            metta.run(
                """
        ...
        metta.register_atom(
            r"id_poly_u", OperationAtom("id_poly_u", lambda x: x, ["$t", "$t"])
        )
        ### Undefined arguments
        ...
        metta.register_atom(
            r"id_undef",
            OperationAtom(
                "id_undef", lambda x: x, [AtomType.UNDEFINED, AtomType.UNDEFINED]
            ),
        )
        self.assertEqual(
            metta.run(
