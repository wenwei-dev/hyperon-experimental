...
class ExamplesTest(HyperonTestCase):
    ...
    def test_self_modify(self):
        ...
        self.assertEqualMettaRunnerResults(
            metta.run("!(get-state (name id-001))"), [[S("Fritz")]]
        )
        metta.run("!(change-state (name id-001) Sam)")
        self.assertEqualMettaRunnerResults(
            metta.run("!(get-state (name id-001))"), [[S("Sam")]]
        )
    ...
    def test_new_object(self):
        ...
        self.assertEqual(
            target.get_children()[1].get_children()[1].get_object().value, 5
        )
    ...
    def test_frog_reasoning(self):
        ...
        self.assertEqualMettaRunnerResults(
            [metta.parse_all("(= (Fritz green) True)")],
    ...
    def test_plus_reduces_Z(self):
        ...
        self.assertEqualMettaRunnerResults(
            ...
            [[ValueAtom(True)], metta.parse_all("(eq 5 4)"), [ValueAtom(True)]],
        )
    ...
    def test_multi_space(self):
        ...
        self.assertEqualMettaRunnerResults(
            metta2.run("!(find-in &space1 B)"), [[S("A")]]
        )
        ...
        self.assertEqualMettaRunnerResults(
            metta2.run("!(borrow &space1 (inverse B))"), [[S("A")]]
        )
        ...
        self.assertEqualMettaRunnerResults(
            metta2.run("!(borrow &space1 (how-it-works?))"), [[S("success")]]
        )
        self.assertEqualMettaRunnerResults(
            metta1.run("!(how-it-works?)"), [[S("failure")]]
        )
    ...
    def test_custom_deptypes(self):
        ...
        self.assertEqualMettaRunnerResults(
            metta.run("!(:? (HumansAreMortal (Human Socrates)))"), [[]]
        )
    ...
    def test_visit_kim(self):
        ...
        self.assertEqualMettaRunnerResults(
            ...
            [
                metta.parse_all(
                    "(perform (lunch-order Kim)) (perform (health-check Kim))"
                ),
        ...
        self.assertEqualMettaRunnerResults(
            metta.run("!(achieve (visit Kim))"), [metta.parse_all("False True")]
        )
...
def newNewAtom(token, klass, unwrap=True):
    return OperationAtom(
        token, lambda *params: new_atom_op(klass, unwrap, *params), unwrap=False
    )
